...
#Below def no returns rundom numbers for the range of that level
def no():
    rng=rang[i]
    com = rng.find(',')
    rs= int(rng[:com])
...
#Below opsfunc utilises the operator library and randomly picks an operator which is later translated to the actual function
def opsfunc():
    clc=calcs[i]
    if ',' in clc:
        clcs = clc.split(',')
...
print(*message, sep=' ')
message=str(message)
filename = 'results/' + pname +'results.txt'
